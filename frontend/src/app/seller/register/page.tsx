...
export default function SellerRegisterPage() {
    ...
    const { register, handleSubmit, formState: { errors, isSubmitting }, setError } = useForm<SellerRegisterForm>({
    ...
    const onSubmit = async (data: SellerRegisterForm) => {
        try {
            ...
            setError('root', { message: err.response?.data?.message || 'Business Registration failed' });
        }
    };

    return (
        <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4 dark:bg-slate-900 pt-24 pb-12">
            <motion.div
                ...
                <Card className="rounded-3xl shadow-xl shadow-slate-200/50 dark:shadow-none border dark:border-slate-800 overflow-hidden">
                    <CardContent className="p-8">
                        <div className="text-center mb-8">
                            ...
                            <h1 className="text-2xl font-black text-slate-900 dark:text-white">Business Registration</h1>
                            <p className="text-sm text-slate-500 mt-2">Create a seller account and manage your store</p>
                        </div>
                        ...
                        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
                            ...
                            <div className="space-y-4">
                                ...
                                <div className="space-y-2">
                                    <label className="text-xs font-bold uppercase tracking-widest text-slate-400 ml-1 leading-none">Business Email</label>
                                    <div className="relative">
                                        <Input
                                            {...register('email')}
                                            type="email"
                            ...
                            <Button
                                ...
                                type="submit"
                                className="w-full bg-slate-900 text-white h-14 mt-6 rounded-xl font-bold hover:bg-primary-600 transition-all shadow-lg hover:shadow-primary-500/20 disabled:opacity-50"
                            >
